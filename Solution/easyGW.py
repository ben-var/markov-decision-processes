...
def dumpCSV(nIter, times,rewards,steps,convergence,world,method):
    fname = 'out/easy/{} {}.csv'.format(world,method)
...
#    Print the map that is being analyzed
    ...
    #         dumpPolicyMap(MapPrinter.printPolicyMap(allStates, p, gen.getMap()),'out/Value {} Iter {} Policy Map.pkl'.format(world,nIter))
    ...
    #         dumpPolicyMap(MapPrinter.printPolicyMap(allStates, p, gen.getMap()),'out/Policy {} Iter {} Policy Map.pkl'.format(world,nIter))
        ...
        for qInit in [-100,0,100]:
            for epsilon in [0.1,0.3,0.5]:
                ...
                for nIter in iterations:
                    ...
                    if nIter == 50:
                        dumpPolicyMap(MapPrinter.printPolicyMap(allStates, p, gen.getMap()),'out/easy/QL {} {} Iter {} Policy Map.pkl'.format(Qname,world,nIter))
                    if convergence[Qname][-1] < 0.3:
                        simpleValueFunctionVis(agent, p, initialState, domain, hashingFactory, "QL Iteration {}".format(nIter))
                        dumpPolicyMap(MapPrinter.printPolicyMap(allStates, p, gen.getMap()),'out/easy/QL {} {} Iter {} Policy Map.pkl'.format(Qname,world,nIter))
                        break;
                    elif nIter == MAX_ITERATIONS:
                        simpleValueFunctionVis(agent, p, initialState, domain, hashingFactory, "Final QL Iteration {}".format(nIter))
                        dumpPolicyMap(MapPrinter.printPolicyMap(allStates, p, gen.getMap()),'out/easy/Final QL {} {} Iter {} Policy Map.pkl'.format(Qname,world,nIter))
                        break;
